...
![Simulated time series for variants of the Hassell model](fig/Fig3.pdf){ width=65% }
...
- **overcompensation** ($a_1<<0$ coefficient or $\beta>1$) is essential to reproduce fast declines
...
$$N_{t+1,i} = N_{t,i} \frac{\exp(r + \eta_t + \zeta_{t,i})}{(1+\alpha N_{t,i} )^{\beta}}, \; \eta_{t} \sim \mathcal{N}(0,\sigma_T^2), \; \zeta_{t,i} \sim \mathcal{N}(0,\sigma_I^2)$$

(done!) and some refinements for phase-dependence like 

$$N_{t+1,i} = N_{t,i} \frac{\exp(r + \eta_t + \zeta_{t,i})}{(1+\alpha[\text{phase}_t] N_{t,i} )^{\beta[\text{phase}_t]}}, \; \eta_{t} \sim \mathcal{N}(0,\sigma_T^2), \; \zeta_{t,i} \sim \mathcal{N}(0,\sigma_I^2)$$

Still doable to revisit this analysis! Data and more code in the GitHub folder. 


## Questions? 

![_Microtus arvalis_, Photo Adrien Pinot](fig/campa4.JPG){ width=80% }
